<template>
  <v-dialog v-model="dialog" width="1024">
    <template v-slot:activator="{ props }">
      ...
      >
        New
        <v-tooltip open-delay="400" location="bottom" activator="parent"
          >Create new backup</v-tooltip
        >
      </v-btn>
    </template>
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field
                ...
                label="Backup Name"
                persistent-hint
                hint="The name of the backup. Falls back to the global naming schema if you leave it empty."
              ></v-text-field>
            </v-col>
          </v-row>
        </v-container>
...
function generateBackupName() {
  let format =
    cs.config.backupNameFormat ||
    "Full Backup {year}-{month}-{day} {hr24}:{min}:{sec}";
